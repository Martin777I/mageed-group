...
const navItems = [
  ...
  {
    to: '/admin/returns', label: 'المرتجعات',
  ...
  { section: 'الاستيراد' },
  {
    to: '/admin/import', label: 'استيراد المنتجات',
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10" /></svg>,
  },
  {
    to: '/admin/import-history', label: 'سجل الاستيراد',
...
export default function Sidebar() {
  return (
    <aside className="fixed right-0 top-0 h-full w-64 bg-brand-900 border-l border-brand-800/50 flex flex-col z-30">
      {/* Logo */}
          ...
          if (item.section) {
            return (
              <div key={i} className="pt-4 pb-1 px-4">
                <p className="text-xs text-gray-600 font-medium uppercase tracking-wider">{item.section}</p>
              </div>
            );
          }
          return (
      ...
      <div className="p-4 border-t border-brand-800/50">
        <a href="/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-500 hover:text-brand-400 transition-colors">
